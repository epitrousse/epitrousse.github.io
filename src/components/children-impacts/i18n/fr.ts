export const fr = {
  s1: {
    title: `École`,
    p1: `En tant que parent, il est important d’aviser l’école que votre enfant est atteint de l’épilepsie. De plus, l’école devrait mettre en place un plan d’intervention individualisé pour votre enfant. Ce plan inclus divers renseignements tels que l’information sur le type d’épilepsie dont l’élève est atteint, le nom du médecin de l’enfant, sa liste de médicaments et les gestes à poser en cas de crises d’épilepsie.`,
    tellToSchool: {
      p1: `Quoi dire à l’enseignant pour aider mon enfant à l’école ?`,
      bullets: [
        `Rassurer et réconforter l’élève`,
        `Permettre à l’élève de se reposer si nécessaire`,
        `Permettre à l’élève de se rendre aux toilettes s’il y a eu perte des fonctions urinaires et/ou intestinales`,
        `Permettre à l’élève de se changer`,
        `Expliquer à l’élève ce qu’il a manqué en classe`,
        `Expliquer aux autres élèves ce qui s’est passé`,
        `Encourager les camarades à avoir une attitude positive et compréhensive`,
        `Continuer les activités régulières une fois que tout est rentré dans l’ordre`
      ]
    },
    schoolServices: {
      p1: `Les milieux scolaires offrent une panoplie de services pour aider votre enfant dans son cheminement scolaire. Il suffit de discuter des besoins de votre enfant auprès de son professeur. Voici une liste de services disponibles :`,
      bullets: [
        `Service de prise de note`,
        `Temps supplémentaire aux examens`,
        `Accès à un local adapté`,
        `Horaire adapté selon la prise de médication`,
        `Soutien pédagogique adapté (ex.: tutorat)`
      ]
    },
    p2: `Une question à laquelle votre enfant peut être confronté dans la cour de récréation est celle de savoir si les enfants atteints d’épilepsie sont moins intelligents ? La réponse à cette question est non. Les enfants qui ont l’épilepsie sont en moyenne aussi intelligents que les autres enfants. Certains enfants ayant l’épilepsie peuvent toutefois présenter des difficultés d’apprentissage en raison de la récurrence des crises ou à causes des effets indésirables des médicaments (ex.: somnolence, fatigue).`
  },
  s2: {
    title: `Psychosocial`,
    p1: `L’épilepsie peut avoir un impact psychosocial sur l’enfant. En effet, un enfant qui vit avec l’épilepsie peut ressentir du stress, avoir une faible estime de soi et/ou vivre de la colère. En effet, l’imprévisibilité des crises peuvent susciter beaucoup de stress chez l’enfant. La réaction de l’entourage peut également avoir un impact sur le stress vécu par l’enfant. Un entourage qui réagit de façon calme, rassurante et réconfortante plutôt que de manière agitée, aidera l’enfant à s’adapter à ce qu’il vit. D’ailleurs, un parent qui aide son enfant à voir l’épilepsie de façon positive et qui parle ouvertement de l’épilepsie avec son enfant, évitera de créer un tabou entourant la maladie.`,
    promoteWellBeing: {
      question: `Comment promouvoir le bien-être de votre enfant? Garder un rythme de vie NORMALE!`,
      bullets: [
        `Encourager son enfant à avoir une vie sociale active`,
        `Encourager son enfant à participer à des loisirs et des activités sportives`,
        `Encourager son enfant à participer aux tâches domestiques à la maison`,
        `Ne pas surprotéger son enfant et lui laisser de l’autonomie`
      ]
    },
    p2: `Lorsque vous apprenez que votre enfant a l’épilepsie, il se peut que vous soyez secoués, que vous éprouviez de la colère, du découragement, de l'impuissance, de la honte et même de la culpabilité. Vous vous demandez peut-être pourquoi cela arrive à votre enfant. Il faut se rappeler que ces réactions sont normales et qu'elles finiront par passer`,
    p3: `Truc: N’hésitez pas à faire appel à des ressources d’aide et surtout, parlez-en à votre médecin, il est aussi disponible pour accueillir vos émotions et vous offrir un soutien psychosocial.`,
    tips: {
      title: `Trucs pratiques pour améliorer mon quotidien et prévenir les crises`,
      bullets: [
        `Prendre ma médication de façon assidue`,
        `Boire assez d’eau`,
        `Manger sainement et de façon équilibrée`,
        `Limiter ma consommation d’alcool et de drogue`,
        `Prendre des douches plutôt que des bains`,
        `Éviter de verrouiller la porte de la salle de bain`,
        `Être accompagné pour les activités présentant un risque de chute ou qui sont dangereuses (ex. baignade)`
      ]
    }
  },
  s3: {
    title: `Divertissements`,
    p1: `La photosensibilité, c’est à-dire la sensibilité à la lumière produit par les différents appareils électroniques (télévision, cellulaires, etc.) est un déclencheur potentiel de crise. On dit qu’une personne est photosensible si elle a une crise d’épilepsie après avoir été exposé à des scintillements et des clignotants des lumières d’une certaine intensité et à un rythme régulier. Les crises peuvent donc être provoquées par certains jeux vidéos et films chez certains utilisateurs. Cependant, la photosensibilité demeure rare et représente moins de 5 % du 1 % des personnes épileptiques. Cette photosensibilité semble aussi diminuer avec l’âge. Renseignez-vous auprès de votre médecin pour connaître vos risques d’hypersensibilité.`
  }
};

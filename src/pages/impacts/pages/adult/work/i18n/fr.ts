export const fr = {
  title: `Travail`,
  p1: `Plusieurs personnes atteintes d’épilepsie se demandent si elles peuvent travailler. Elles pourront se réjouir de savoir qu’elles peuvent travailler et poursuivre une carrière remplie de succès. Toutefois, il existe des emplois non recommandés aux personnes atteintes d’épilepsie pour des raisons de sécurité personnelle et publique. Si cela correspond à votre situation, nous vous mentionnons que certaines personnes atteintes d’épilepsie ont pu surmonter ces obstacles, par exemple en créant leur entreprise ou en choisissant un milieu plus adapté pour travailler.`,
  p2: `Voici une liste des emplois moins recommandées aux personnes épileptiques ainsi que des conseils généraux pour les travailleurs atteints d’épilepsie :`,
  notRecommendedJob: {
    title: `Emploi peu recommandé:`,
    bullets: [
      `Pilote de ligne`,
      `Contrôleur aérien`,
      `Chef de train`,
  ...
  generalTips: {
    ...
    bullets: [
      `Horaire de travail régulier et stable`,
      `Toujours porter un équipement de protection en cas de situation dangereuse`,
      `Bonne adhérence aux médicaments`,
      `Prise de connaissance des effets secondaires aux médicaments, et comment cela peut affecter le rendement du travail.`,
  ...
  ss1: {
    ...
    bullets: [
      `Il est important de comprendre les risques engendrés par un emploi. Pour ce faire, on peut les étudier de manière approfondie et obtenir des conseils du spécialiste en adaptation au Conseil canadien de la réadaptation et du travail (CCRT). Pour ce faire, vous pouvez téléphoner au 1-800-664-0925 ou au 416-260-3060. Voici le lien pour accéder à leur site Web : https://ccrw.org/fr/.`,
      `On peut informer nos collègues de notre condition et des premiers soins à donner en cas de crise. Pour ce faire, vous pouvez recevoir l’aide de l’Alliance canadienne de l’épilepsie de votre région.`,
      `Avoir un horaire de travail régulier et éviter de passer beaucoup d’heures sans dormir.`,
      `Si vous travaillez en hauteur, il est important de s’assurer de monter à des hauteurs où le fait de tomber est sans danger. Sinon, assurez-vous d’être protégés par un harnais de sécurité et un casque.`,
      `Avoir des vêtements de rechange au travail, car vos vêtements peuvent être souillés lors d’une crise.`,
      `Si vous travaillez à proximité de machinerie, il est important de vérifier régulièrement les fonctions de sécurité ou les dispositifs de sécurité.`
    ]
  }
};
